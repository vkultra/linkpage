        ...
        return (
          <div key={link.platform} className="flex items-center gap-2">
            ...
            <input
              ...
              className="flex-1 rounded-lg border border-gray-200 px-3 py-1.5 text-sm text-gray-900 focus:border-gray-400 focus:outline-none dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:focus:border-slate-400"
            />
            <button
              type="button"
              onClick={() => removeLink(link.platform)}
              className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full text-gray-400 hover:bg-red-50 hover:text-red-500 dark:text-slate-400 dark:hover:bg-red-500/10 dark:hover:text-red-400"
            >
          ...
          <div className="flex flex-wrap gap-1.5">
            {availablePlatforms.map((platform) => (
              <button
                ...
                className="flex items-center gap-1.5 rounded-full border border-gray-200 px-2.5 py-1 text-xs text-gray-600 transition-colors hover:border-gray-300 hover:bg-gray-50 dark:border-slate-600 dark:text-slate-300 dark:hover:border-slate-500 dark:hover:bg-slate-700"
              >
...
function AddLinkForm({
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 p-2 dark:border-slate-700/50 dark:bg-slate-700/30">
      <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full" style={{ backgroundColor: platform.color + '1a' }}>
        <svg className="h-4 w-4" viewBox="0 0 24 24" fill={platform.color}>
          <path d={platform.icon} />
        </svg>
      ...
      <button
        type="button"
        onClick={() => onAdd(url)}
        className="rounded-lg bg-gray-900 px-3 py-1.5 text-xs font-medium text-white hover:bg-gray-800 dark:bg-slate-50 dark:text-slate-900 dark:hover:bg-slate-200"
      >
        Adicionar
      </button>
      <button
